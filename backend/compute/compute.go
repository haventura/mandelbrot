...
func main() {
	...
	for x := 0; x < width; x++ {
		...
		for y := 0; y < height; y++ {
			y := y
			go worker(x, y, width, height, min_r, max_r, min_i, max_i, max_it, data_c)
		}
	}
	wg.Wait()
	close(data_c)
	...
	start_rend := time.Now()
	render(width, height, data_c, file_name, "mycolormap")
	elapsed_rend := time.Since(start_rend)
	fmt.Printf("Rendering took %s", elapsed_rend)
}

func worker(x int, y int, width int, height int, min_r float64, max_r float64, min_i float64, max_i float64, max_it int, data_c chan<- point) {
	defer wg.Done()
	r := scale_px_to_coord(x, width-1, min_r, max_r)
	i := scale_px_to_coord(y, height-1, min_i, max_i)
	c := complex(r, i)
	z, it := mandelbrot(c, max_it)
...
func mandelbrot(c complex128, max_it int) (complex128, int) {
